...
class RefreshTokenRepository:
    ...
    async def create(self, token_hash: str, user_id: int, expires_days: int = REFRESH_TOKEN_EXPIRE_DAYS) -> RefreshTokenModel:
        expires_at = datetime.utcnow() + timedelta(days=expires_days)
        rt = RefreshTokenModel(token_hash=token_hash, user_id=user_id, expires_at=expires_at)
        self.db.add(rt)

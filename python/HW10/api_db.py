...
async def average_score_by_faculty(user_id: UUID, user_data: UserUpdate, db_manager: DataBaseManager = Depends(get_manager)) -> List[User]:
    try:
...
async def delete_user(user_id: UUID, db_manager: DataBaseManager = Depends(get_manager)) -> List[User]:
